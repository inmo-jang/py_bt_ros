#!/bin/bash
# Run nav_action_server.py for N Limo robots in parallel.
# Usage: ./run_action_servers.sh [NUM_ROBOTS]
#   NUM_ROBOTS: number of robots to launch (default: 10)
#
# Run from the project root:
#   bash scenarios/example_simple_limo/scripts/run_action_servers.sh
#   bash scenarios/example_simple_limo/scripts/run_action_servers.sh 3

NUM_ROBOTS=${1:-10}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../../.." && pwd)"
NAV_SERVER="$ROOT_DIR/scenarios/example_simple_limo/action_servers/nav_action_server.py"
